...
These are the implementation assumptions, tradeoffs, and known weak spots behind the current design.

## What I assumed

**Output is grounded, not self-certifying.** Titan optimises for source-supported drafting. I prioritised the citation pipeline (chunk-level provenance, inline `[doc_id:page:span]` tags, post-hoc overlap check) instead of trying to build something that emits legally defensible language without operator review.
...
**Artifacts should stand on their own.** The README, `examples/`, and `eval/` directories are written so the learning loop can be inspected from the diff between `examples/output_v1.json`, `examples/edited_v1.json`, and `examples/output_v2.json` without installing dependencies.

## What I deliberately didn't do

**No fine-tuning.** I could have set up DPO over the simulated edits, but 24 edits is too little signal for an inspectable model update. Retrieval-based learning is auditable: the rule YAMLs are human-readable, the few-shots are concrete before/after pairs, and you can trace which edit fired into which draft.

**No production Qwen2.5-VL call.** The page classifier routes handwritten pages to a `_run_qwen2_5_vl` function. That function is stubbed in this build — it expects a transcript fixture at `data/gold/<doc>.transcript.md`. The hook is one HTTP call away from working; the 1875 handwritten deed runs end-to-end via the fixture path, so the rest of the pipeline is exercised on genuinely messy input.
...
**No multi-tenant story.** Everything is scoped to a single workspace and matter. `matter_id` exists in the schema and gets persisted, but there's no operator authentication, no row-level security, no isolation between matters. That is acceptable for a local workflow and incomplete for production.
...
**Eight ALTA sections vs. a free-form draft.** A free-form summary would have been faster to write and easier to demo. A schema-driven draft is more constrained but easier to evaluate quantitatively (per-section edit distance, rule application rate per section, etc). I picked the stricter path because measurable grounding matters for this product shape.

**Three docs vs. ten.** I started with eight sample documents across six categories, then narrowed the eval set to leave enough focus for the learning loop and the docs. The remaining documents are still in `data/raw/` for anyone who wants to run the pipeline on a broader corpus.
